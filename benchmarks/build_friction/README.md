...
`build_friction.py` → `results_2026-09-05.json` (`measured:true`), macOS arm64 (earlier snapshots: `results_2026-08-29.json`).

| Language | Toolchain (probe) | Hello-World → .wasm | Error Class (measured verbatim) |
|---|---|---|---|
| **Rust** | cargo 1.92 + wasm32-wasip1 installed | ✅ **0.4 s** (2026-09-05, warm toolchain cache; the 2026-08-29 snapshot recorded 0.1 s — cache-dependent) | — |
...
→ .wasm 0.4 s (2026-09-05 measurement, see results JSON; the gate-time
measurement in ADR-005/CHANGELOG cites 2.9 s from a cold-cache run),
executed in the cell; Go in CI.
